...
function showCanvas(canvasDiv,canvasImg){
    ...
    var lineSizeSelect = div.children('.lineSizeSelect');
    var lineCalorSelect = div.children('.lineColorSelect');
    var cancelButton = div.children('.canvas-cancel-button');

    pen.click(writeMode);
    eraser.click(eraseMode);
    lineSizeSelect.change(changeLineSize);
    lineCalorSelect.change(changeLineColor);
...
function saveCanvas(url,canvasImage,canvasDiv,name,width,height){
    ...
    complete:function(e){
        ...
        if(myDiv.children('.readmore').attr('checked')){
            var readmores = jQuery('.readmore');
            readmores.attr({'checked':''});
            myDiv.children('.readmore').attr({'checked':'checked'});
        }
...
function changeLineColor(){
    ...
    lineColorHold = me.val();
    var className = me.attr('class');
    var classes = jQuery('.' + className);
    classes.each(function(){
        jQuery(this).val(lineColorHold)
    });

    me.prev().css('color',lineColorHold);
...
function cancelCanvas(myDiv){
    ...
    jQuery.unblockUI();
    myDiv.css({'display':'none'});
    var readmore = myDiv.children('.readmore');
    
    if(readmore.attr('checked') && myDiv.children('.my-html').val().match('')){
        readmore.attr({'checked':''});
    }
